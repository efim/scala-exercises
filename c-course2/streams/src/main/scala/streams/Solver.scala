...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    ...
    b.legalNeighbors.toStream.map((pair:(Block, Move)) => (pair._1, pair._2::history))
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from(Stream((startBlock, List.empty[Move])), Set())
